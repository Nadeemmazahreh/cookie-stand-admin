...
function Main(props) {
    ...
    function minimumHandler(event) { 
        Minimum(event.target.value); 
    }

    function maximumHandler(event) { 
        Maximum(event.target.value); 
    }

    function averageHandler(event) { 
    ...
    function create(event) {
        ...
        for (let i = 0; i < 14; i++) {
            ...
            data.stand.push(total)
            
            for (let j = 0; j < report.length + 1; j++) {
                
                total += report[j] ? report[j].stand[i] : 1
            }
            result.push(total)
        }
    ...
    return (
        <main className="grid w-full px-.25 text-center bg-green-100 p-11 justify-items-stretch">
            <form className="py-5 bg-green-300 rounded mx-11 px-9 w-4/5justify-self-center" onSubmit={create}>
                <fieldset>
                    ...
                    <div className="flex flex-col ...">
                        ...
                        <div className="container mx-auto w-10/12 my-1.5" >
                            <div className="flex space-x-7 ...">
                                ...
                                <div className="flex flex-col ... w-1/5 text-xs text-center bg-green-100 p-1.5 rounded-lg">
                                    <label > Average stand per Sale </label>
                        ...
                        <tbody className="border border-collapse border-gray-900">
                            {report.map(data => (
                                <tr className="border border-collapse border-gray-900" key={data.ID}>
                                    ...
                                    {data.stand.map(cookie => (<td className="border border-collapse border-gray-900"> {cookie} </td>))}
                                    <td className="border border-collapse border-gray-900">{data.stand.reduce((account, current) => { account = account + current; return account }, 0)}</td>
                                </tr>
                            ))}
                        </tbody>
